...
let s:open_patt = '\C\%(\<\%(fn\|if\|defer\|try\|do\)\>\|(\|{\)'
let s:middle_patt = '\C\<\%(else\|elseif\|after\|aftertry\|catch\)\>'
let s:close_patt = '\C\%(\<\%(end\)\>\|)\|}\)'

let s:anon_func_start = '\S\+\s*[({].*\<fn\s*(.*)\s*$'
let s:anon_func_end = '\<end\%(\s*[)}]\)\+'

let s:chained_func_call = "^\\v\\s*[:.]\\w+[({\"']"

" Expression used to check whether we should skip a match with searchpair().
let s:skip_expr = "synIDattr(synID(line('.'),col('.'),1),'name') =~# 'trefleComment\\|trefleString'"

" Auxiliary Functions -------------------------------------{{{1

function s:IsInCommentOrString(lnum, col)
  return synIDattr(synID(a:lnum, a:col, 1), 'name') =~# 'trefleCommentLong\|trefleStringLong'
...
" GetTrefleIndent Function -----------------------------------{{{1

function GetTrefleIndent()
